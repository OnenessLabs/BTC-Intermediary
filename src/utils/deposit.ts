...
async function deposit(amount: string, token: string) {
    ...
    if(useToken!==ETH_ADDRESS) {
        const increaseAllowanceTx = await tokenContract.populateTransaction.approve(swapContract.contract.address, amountBN);
        increaseAllowanceTx.gasLimit = BigNumber.from(100000);
        await EVMSigner.sendTransaction(increaseAllowanceTx);
    }

    const depositTx = await swapContract.contract.populateTransaction.deposit(useToken, amountBN);
    if(useToken===ETH_ADDRESS) depositTx.value = amountBN;
